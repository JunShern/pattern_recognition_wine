...
chebychev = @(A,B)pdist2(A,B,'chebychev');
...
%             [cluster_indices,centroid_data] = kmeans_custom(train_data, K, chebychev);
%         elseif strcmp(dist, 'intersection')
%             [cluster_indices,centroid_data] = kmeans_custom(train_data, K, intersection);
%         elseif strcmp(dist, 'kullbackleibler')
%             [cluster_indices,centroid_data] = kmeans_custom(train_data, K, kullbackleibler);
%         elseif strcmp(dist, 'jensenshannon')
%             [cluster_indices,centroid_data] = kmeans_custom(train_data, K, jensenshannon);
%         elseif strcmp(dist, 'chisquare')
%             [cluster_indices,centroid_data] = kmeans_custom(train_data, K, chisquare);
%         elseif strcmp(dist, 'earthmovers')
%             [cluster_indices,centroid_data] = kmeans_custom(train_data, K, earthmovers);
        else
            [cluster_indices,centroid_data] = kmeans(train_data, K);
        end
...
saveas(gcf,'knn_with_clustering_allsamedistclusters_errors.png')
